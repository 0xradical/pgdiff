...
# SELECT pgdiff.dependency_tree('');

# -- Dependencies for any relations with names containing match (in regular expression)
# SELECT pgdiff.dependency_tree('match');

# -- Dependencies for relations person & address
# SELECT pgdiff.dependency_tree('{person,address}'::text[]);

# -- Dependencies for function slice
# SELECT pgdiff.dependency_tree(ARRAY['slice'::regproc]);

# -- Dependencies for type hstore
# SELECT pgdiff.dependency_tree(ARRAY['hstore'::regtype]);

# -- Dependencies for triggers by the name updated
# SELECT pgdiff.dependency_tree(ARRAY(
  ...
  class Deps
    ...
    def setup
      query(%{
        ...
        CREATE OR REPLACE FUNCTION pgdiff.dependency_tree(search_pattern text)
          RETURNS TABLE(dependency_tree text)
          SECURITY DEFINER LANGUAGE SQL
          AS $function$
        WITH target AS (
          SELECT objid, dependency_chain
          FROM pgdiff.dependency
        ...
        , list AS (
          ...
          FROM target
          JOIN pgdiff.dependency pgdiff
            ON pgdiff.objid = ANY(target.dependency_chain) -- root-bound chain
            OR target.objid = ANY(pgdiff.dependency_chain) -- leaf-bound chain
        ...
        -- Procedure to pgdiff depedency tree by specific relation name(s) (in text array)
        CREATE OR REPLACE FUNCTION pgdiff.dependency_tree(object_names text[])
          RETURNS TABLE(dependency_tree text)
          SECURITY DEFINER LANGUAGE SQL
          AS $function$
        WITH target AS (
        ...
        -- Procedure to pgdiff depedency tree by oid
        CREATE OR REPLACE FUNCTION pgdiff.dependency_tree(object_ids oid[])
          RETURNS TABLE(dependency_tree text)
          SECURITY DEFINER LANGUAGE SQL
          AS $function$
        WITH target AS (
        ...
        , list AS (
          SELECT DISTINCT
            format('%*s%s %s', -4*level
                  , CASE WHEN pgdiff.objid = ANY(object_ids) THEN '*' END
                  , object_type, object_identity
            ) AS dependency_tree
          , dependency_sort_chain
          FROM target
